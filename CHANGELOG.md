...
- Scan omp session logs. `scan-agent-logs` read Claude and Codex only, so omp — which inherits toolsmith's MCP registration from those same configs — was completely invisible: it could run all day, pull whole large files into context, and report nothing. It now scans `~/.omp/agent/sessions`, counting native `read`/`edit`/`write` on files over 200 lines, its `bash` commands through the existing shell classifier, and any `mcp__toolsmith__*` calls as adoption. Reads that already pass `offset`/`limit` are not counted against it, and `skill://` pseudo-paths are ignored because omp loads skills through the same read tool. First scan surfaced 504 lost opportunities over 14 days that nothing had been measuring.

- Prime omp, which already had the tools and was never told to use them. omp imports MCP servers from the Claude and Codex configs, so it inherits toolsmith's registration without any setup — and the telemetry showed it: 42 tool-list requests, `toolCount: 9`, and zero tool calls. The priming block that tells an agent to prefer `mcp__toolsmith__*` only ever reached `~/.claude/CLAUDE.md`, `~/.codex/AGENTS.md`, `~/.gemini/GEMINI.md`, and `AGENTS.md`, never omp's own `~/.omp/agent/AGENTS.md`. `setup`/`adopt --inject` now covers it when omp is installed. No MCP registration is written for omp: it already has one, and a second would be a duplicate. The target list also moves into one `primingTargets()` used by both inject and remove, deduped, instead of being spelled out twice.

- Stop forcing a re-read after every edit. Toolsmith was spending ~9.2 calls per edited file against a native equivalent of ~4.6, and 60% of all navigation happened *after* the first edit — the agent editing, then re-reading to orient itself again. That re-read was never necessary: anchors are reconciled by LCS, so 99.8% of them survive an edit and an anchor captured before an edit still resolves after it. The agent simply had no way to know, because a successful edit answered `Applied 1 anchored edit(s)` and nothing else. Edits now return the changed lines anchored and ready to use, and say plainly that the anchors already held remain valid. On a 2,000-line file that reply costs 207 tokens where the re-read it replaces cost 37,445, and it removes a model round trip. Applies to `anchored_edit`, `anchored_edit_many`, and `symbol_replace`, bounded to 40 lines (24 for batches) and shared across regions so scattered edits stay cheap.

- Fix the round-trip baseline in `toolsmith trends`. It compared every edited file against a flat two native calls, which counted legitimate work as overhead: a file that genuinely takes six edits takes six native `Edit` calls too. The baseline is now one orienting read plus one edit per change, which moves the reported overhead from 7,146 excess calls to 5,305 and stops the metric overstating its own problem.
