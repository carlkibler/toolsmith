...
export async function runDoctor() {
  ...
  if (list === null) {
    ...
    if (claudeGet) {
      ...
      if (drift.state === "ok") ok(drift.message || "Claude Code: command points at this checkout")
  ...
  if (geminiList === null) {
    ...
    if (geminiConfig?.error) {
      ...
      if (drift.state === "ok") ok(drift.message || "Gemini CLI: command points at this checkout")
  ...
  if (!existsSync(path.join(homedir(), ".codex"))) {
    ...
    if (drift.state === "ok") ok(drift.message || "Codex: command points at this checkout")
  ...
  for (const client of doctorJsonMcpClients(expectedMcpPath)) {
    if (client.state === "missing") {
      ...
      ok(client.message || `${client.label}: command points at this checkout`)
  ...
  if (wire) {
    // End-to-end delivery proof: the smoke test shows the server lists tools; the wire
    // canary shows a client actually RECEIVES anchored content on both delivery channels.
    // Exits non-zero on failure so a cron/reminder can alert instead of a human noticing
    // weeks later (the 0.1.53 structuredContent regression ran 29 days undetected).
    section("\nwire canary (end-to-end content delivery)")
    const verdict = await wireCanary()
    if (verdict.ok) ok(`wire canary passed in ${verdict.ms}ms — ${verdict.detail}`)
    else { warn(`wire canary FAILED — ${verdict.detail}`); process.exitCode = 1 }
  }
