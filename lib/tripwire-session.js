...
// (any Toolsmith call resets the count — see resetSession). It gets firmer the longer it's
// ignored: nudge → ask. It deliberately caps at "ask" and never auto-denies — hard blocking is
// opt-in via a fixed --tripwire-mode deny. Per-session so a fresh session always starts gentle.
...
// Reset a session's bypass count — called whenever the agent uses a Toolsmith tool, so the
// counter measures CONSECUTIVE ignoring (native ops with no Toolsmith use between), not total
// large-file work. An agent that's actually using Toolsmith never escalates. Best-effort.
