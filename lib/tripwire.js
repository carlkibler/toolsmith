...
export const TRIPWIRE_MODES = new Set(["allow", "ask", "deny"])
...
export function evaluateTripwire(data = {}, options = {}) {
  ...
  if (["Read", "Edit", "MultiEdit"].includes(toolName)) {
    ...
    const message = toolName === "Read"
      ? `Toolsmith tripwire: ${info.displayFile} is ${info.lines} lines. Map it with mcp__toolsmith__file_skeleton, then pull only what you need via mcp__toolsmith__anchored_read (startLine/endLine) or mcp__toolsmith__get_function — don't read the whole file into context.`
      : `Toolsmith tripwire: ${info.displayFile} is ${info.lines} lines. Find the spot with mcp__toolsmith__find_and_anchor, then change it with mcp__toolsmith__anchored_edit (or mcp__toolsmith__symbol_replace for one function/class) — validated, surgical, no whole-file rewrite.`
...
export async function runTripwire() {
  ...
  if (sub === "run") {
    ...
    try {
      ...
      // Piggyback an "update available" hint here — the tripwire fires exactly when an
      // outdated toolsmith is most likely the cause of native-tool misuse. Cache-only, instant.
      try {
      ...
      const mode = option("--mode") || process.env.TOOLSMITH_TRIPWIRE_MODE || "allow"
