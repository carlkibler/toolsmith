...
import { packageInfo, stableNodeCommand, wireCanary } from "./config.js"
import { updateNoticeSuffix } from "./update-check.js"
import { adaptiveMode, recordFire, resetSession, pruneOldSessions, markEditOnramp, readWireVouch, recordWireVouch } from "./tripwire-session.js"
...
export function primeText() {
  return "Toolsmith reminder: use mcp__toolsmith__* for broad reads or edits in files over ~200 lines. Native Read with an explicit small limit (≤300 lines) is fine for inspection; switch to anchored_read/anchored_edit before changing those lines so you avoid both token waste and rejected-tool retry turns."
}
...
// First-edit on-ramp: the FIRST native large-file edit/write of a session gets this distinct,
// louder message instead of the repeated nudge that gets tuned out. It names the gait so the
// agent sees the whole-session cost, not just this one edit. Forensics finding: identical
// per-edit nudges converted ~200→0 in heavy build sessions; a one-time, different first message
// is the leverage point — the gait locks at the first edit, not the hundredth.
const ONRAMP_EDIT_IDS = new Set(["native-edit-large-file", "native-multiedit-large-file", "native-write-large-file"])

function onrampEnabled() {
  return process.env.TOOLSMITH_TRIPWIRE_ONRAMP !== "0"
}

function onrampEditMessage(info) {
  return `⚓ Toolsmith on-ramp — first large native edit this session (${info.displayFile}, ${lineSummary(info)}, ~${tokenCostK(info)}K tokens to rewrite whole). A native Read→Edit gait tends to repeat for the whole session and rewrites regions without validating anchors. Switch now: mcp__toolsmith__find_and_anchor to locate the spot, then mcp__toolsmith__anchored_edit (or mcp__toolsmith__symbol_replace for one symbol) — validated, surgical, no rejected-tool retry turn. Do it once and the rest of the session inherits the cheaper, safer path.`
}
...
export function evaluateTripwire(data = {}, options = {}) {
  ...
  if (["Edit", "MultiEdit"].includes(toolName)) {
    ...
    const message = `Toolsmith tripwire: ${info.displayFile} is ${lineSummary(info)} (~${costK}K tokens). Find the spot with mcp__toolsmith__find_and_anchor, then change it with mcp__toolsmith__anchored_edit (or mcp__toolsmith__symbol_replace for one function/class) — validated, surgical, no whole-file rewrite, and no rejected-tool retry turn.`
...
export function logTripwireFinding(findingResult, meta = {}) {
...
// Adaptive is turn-cost-aware: it only escalates repeated native EDIT/WRITE bypasses.
// Reads and shell inspection stay as nudges because rejecting them often costs an extra
// model round trip for little safety gain. A Write that CREATES a new file is also a
// pure nudge — only Write can create it, so escalation has no valid alternative.
function adaptiveCeiling(result) {
  const id = result?.id || ""
  if (!["native-edit-large-file", "native-multiedit-large-file", "native-write-large-file"].includes(id)) return "allow"
...
export function summarizeTripwireLog({ sinceMs = 0, logPath = tripwireLogPath() || DEFAULT_LOG } = {}) {
  const summary = { logPath, total: 0, byId: {}, byMode: {}, byDecision: {}, firmPrompts: 0, latestTs: null }
  ...
  for (const line of lines) {
    try {
      ...
      const decision = row.decision || row.mode
      if (decision) {
        summary.byDecision[decision] = (summary.byDecision[decision] || 0) + 1
        if (["ask", "deny"].includes(decision)) summary.firmPrompts += 1
      }
...
export function runTripwireStatus() {
  ...
  if (summary.firmPrompts) console.log(`Firm prompts / estimated retry turns: ${summary.firmPrompts} (${Object.entries(summary.byDecision).map(([k, v]) => `${k}=${v}`).join(", ")})`)
}

// Resolve (and cache) whether this install's MCP server verifiably delivers content.
// TOOLSMITH_TRIPWIRE_VOUCH=0 opts out (always vouch, pre-0.1.56 behavior). Any canary
// error counts as a failure: if the server can't even start, recommending it is wrong.
async function tripwireVouch() {
  if (process.env.TOOLSMITH_TRIPWIRE_VOUCH === "0") return true
  const version = packageInfo().version || "0.0.0"
  const cached = readWireVouch(version)
  if (cached) return cached.ok
  const verdict = await wireCanary()
  recordWireVouch(version, verdict.ok, verdict.detail)
  return verdict.ok
}
...
export async function runTripwire() {
  ...
  if (sub === "run") {
    ...
    try {
      ...
      // The tripwire only vouches for what it has verified: before recommending Toolsmith,
      // prove end to end (cached per install version — see readWireVouch) that the MCP server
      // delivers anchored content to clients. Without this, a delivery regression like
      // 0.1.53's structuredContent strip turns every nudge into a push toward a broken tool
      // and costs the agent a discovery round trip per fire. On failure: log and go silent —
      // never block, never recommend. Fires only when a finding exists, so the common
      // no-finding path pays nothing.
      if (!(await tripwireVouch())) {
        logTripwireFinding(result, { requestedMode: requested, mode: "allow", decision: "silenced-unverified" })
        return
      }
      // Resolve firmness. Default is nudge-only "allow". Opt-in adaptive counts this
      // session's bypasses (reset whenever the agent uses a Toolsmith tool) and escalates
      // repeated edit/write bypasses to ask. Adaptive never auto-denies.
      let mode = requested
      let fires = 0
      ...
      const reachable = reachableByToolsmith(result)
      if (!reachable) mode = "allow"
      ...
      const decision = mode === "allow" ? "nudge" : mode
      logTripwireFinding(result, { requestedMode: requested, mode, decision, fires })
      if (requested === "adaptive" && mode !== "allow") {
        result.message += ` Toolsmith was bypassed ${fires}× this session — now requiring it (${mode}). This costs a retry turn now, but prevents repeated large native edits.`
      }
      // First-edit on-ramp (all modes, pure message change — never alters the decision): the first
      // native large-file EDIT/WRITE of a session swaps the repeated nudge for a distinct one that
      // names the gait, then goes quiet. Skips brand-new file writes (no result.lines) and files
      // Toolsmith can't reach. Re-arms when the agent uses Toolsmith (PostToolUse reset clears it).
      if (onrampEnabled() && sessionId && ONRAMP_EDIT_IDS.has(result.id) && result.lines
          && reachable && markEditOnramp(sessionId)) {
        result.message = onrampEditMessage(result)
        touchedSession = true
      }
      if (touchedSession) pruneOldSessions()
