...
function formatFinding(findingResult, format) {
  ...
  // Emit only documented PreToolUse hook output fields. The user-facing nudge
  // belongs in top-level `systemMessage`; `permissionDecisionReason` carries the
  // decision rationale. `systemMessage` is NOT a documented `hookSpecificOutput`
  // field, so keeping it nested risks breaking if Claude Code tightens validation
  // of that object the way it now rejects an invalid top-level `decision`.
