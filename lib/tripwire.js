...
export function evaluateTripwire(data = {}, options = {}) {
  ...
  if (toolName === "Read") {
    ...
    const message = `Toolsmith tripwire: ${info.displayFile} is ${info.lines} lines (~${costK}K tokens to read whole). Map it with mcp__toolsmith__file_skeleton, then pull only what you need via mcp__toolsmith__anchored_read (startLine/endLine) or mcp__toolsmith__get_function — a targeted read is a fraction of that.`
    return finding("native-read-large-file", toolName, message, info)
  }
  ...
  if (["Edit", "MultiEdit"].includes(toolName)) {
    ...
    if (!info) return null
    const costK = tokenCostK(info)
    const message = `Toolsmith tripwire: ${info.displayFile} is ${info.lines} lines (~${costK}K tokens). Find the spot with mcp__toolsmith__find_and_anchor, then change it with mcp__toolsmith__anchored_edit (or mcp__toolsmith__symbol_replace for one function/class) — validated, surgical, no whole-file rewrite.`
