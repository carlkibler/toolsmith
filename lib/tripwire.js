...
export function largeFileInfo(filePath, cwd, maxLines = DEFAULT_MAX_LINES) {
  ...
  // Is the file inside the workspace Toolsmith could actually edit? (Its MCP tools refuse paths
  // outside cwd.) The tripwire uses this to avoid blocking files Toolsmith can't reach.
  const rel = path.relative(cwd || process.cwd(), resolved)
  const contained = rel !== "" && !rel.startsWith("..") && !path.isAbsolute(rel)
  return { file: resolved, displayFile: redactedHome(resolved), lines, size, contained }
}
...
// Cap escalation by what the agent can actually do instead. Reads never hard-block (the agent
// may legitimately need a full read of a file Toolsmith can't parse); a large Write to a NOT-
// YET-EXISTING file never blocks (Write is the only way to create a file — anchored_edit needs
// one). Edits and writes to existing large files can escalate all the way to deny.
// Can Toolsmith actually read/edit this file? Its MCP tools refuse paths outside the workspace
// and files over the read limit. If not, NO mode may block it — blocking with no reachable
// alternative is a catch-22. This guard applies to every mode, including a fixed --mode deny.
function reachableByToolsmith(result) {
  if (result?.contained === false) return false
  if (result?.size && result.size > TOOLSMITH_MAX_BYTES) return false
  return true
}

// Adaptive-only softening: reads never hard-block (the agent may need a full read of a file
// Toolsmith parses poorly, e.g. a minified bundle); a Write that creates a new file never blocks
// (only Write can create it). Edits/existing-file writes can escalate to deny.
function adaptiveCeiling(result) {
  const id = result?.id || ""
  if (id.startsWith("native-read") || id.startsWith("shell-")) return "ask"
