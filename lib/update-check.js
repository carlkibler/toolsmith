...
// The right update command for HOW this copy was installed. The Homebrew tap is retired,
// so a brew copy can never reach a newer version — tell those users to migrate to npm
// rather than `toolsmith update`, which would shell out to npm and silently shadow it.
export function updateCommand(kind) {
  if (selfUnderHomebrew()) return "brew uninstall carlkibler/tap/toolsmith && npm install -g @carlkibler/toolsmith"
