...
// One-line human notice, install-kind aware, or null when up to date / unknown.
export function updateNoticeText(currentVersion, { kind } = {}) {
  ...
  const how = kind && String(kind).startsWith("git") ? "git pull" : "toolsmith update"
  const brew = process.env.HOMEBREW_PREFIX ? " · or: brew upgrade carlkibler/tap/toolsmith" : ""
  return `Toolsmith ${status.current} → ${status.latest} available. Run: ${how}${brew}`
}
...
export function updateNoticeSuffix(currentVersion) {
  ...
  return ` · toolsmith ${status.current}→${status.latest} available (run: toolsmith update)`
}
