...
import { shellProvenanceHeader, provenanceTag } from "./provenance.js"
import { stableNodeCommand } from "./config.js"

export const CODEX_FOOTER_HOOK_RELATIVE = ".codex/hooks/toolsmith-token-footer.sh"
export const CODEX_FOOTER_COMMAND = `bash ~/.codex/hooks/toolsmith-token-footer.sh # ${provenanceTag()}`
...
function codexFooterScript(nodePath = stableNodeCommand()) {
  return `#!/bin/bash
${shellProvenanceHeader("Codex Stop-hook token-savings footer (opt-in via TOOLSMITH_CODEX_FOOTER=1).")}
set -u
...
function scanMisses(transcriptPath) {
  if (!transcriptPath || !fs.existsSync(transcriptPath)) return null
  let sedMisses = 0
  let applyMisses = 0
  let worstFile = null
  try {
    const lines = tailLines(transcriptPath, 1024 * 1024)
    let cwd = homeDir()
    for (const raw of lines) {
      if (!raw) continue
      let row
      ...
      if (payload.type === "function_call" && payload.name === "exec_command") {
        ...
        if (/sed\\s+-n\\s+['"]?\\d+,\\d+p['"]?\\s+\\S/.test(cmd)) {
          sedMisses++
          if (!worstFile) {
            const m = cmd.match(/sed\\s+-n\\s+['"]?\\d+,\\d+p['"]?\\s+(\\S+)/)
            if (m) worstFile = path.basename(m[1])
          }
        }
      } else if (payload.type === "custom_tool_call" && payload.name === "apply_patch") {
        const text = String(payload.input || "")
        for (const m of text.matchAll(/^\\*\\*\\* Update File:\\s+(.+)$/gm)) {
          applyMisses++
          if (!worstFile) worstFile = path.basename(m[1].trim())
        }
      }
    }
  } catch { return null }
  const total = sedMisses + applyMisses
  if (!total) return null
  const what = []
  if (applyMisses) what.push(applyMisses + " apply_patch")
  if (sedMisses) what.push(sedMisses + " shell sed")
  const hint = applyMisses ? "try anchored_edit" : "try anchored_read"
  const onFile = worstFile ? " on " + worstFile : ""
  return "Toolsmith: " + total + " large-file miss" + (total > 1 ? "es" : "") + " this turn (" + what.join(", ") + onFile + " — " + hint + ")"
}
...
const missText = scanMisses(payload.transcript_path || payload.conversation_path)
const mainLine = [savingsText, codexText].filter(Boolean).join(" • ")
if (mainLine) console.log(mainLine)
if (missText) console.log(missText)
...
export function installCodexFooter({ nodePath = stableNodeCommand() } = {}) {
