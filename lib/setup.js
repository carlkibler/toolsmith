...
function installSourceLabel(ctx, { detail = false } = {}) {
  if (ctx.kind === "npm-global") {
    return detail && verboseOutput() && ctx.npmGlobalToolsmithRealPath
      ? `GitHub release package at ${ctx.npmGlobalToolsmithRealPath}`
...
export function runUpdate() {
  ...
  const release = fromPath ? null : githubLatestRelease("carlkibler", "toolsmith")
  const latestVersion = release?.tag?.replace(/^v/, "") || null
  const globalBin = npmGlobalToolsmithBin(ctx)

  function installSummary() {
    return installSourceLabel(ctx, { detail: true })
  }

  function refreshInstalledSetup() {
    if (!refreshSetup) { console.log("\nSkipped client integration refresh (--no-setup)."); return true }
  ...
  if (checkOnly) {
    console.log(`current: v${currentVersion} (${installSummary()})`)
    if (fromPath) {
      console.log(`update source: local path ${path.resolve(fromPath)}`)
    } else if (latestVersion) {
      const dateStr = release.publishedAt ? `, published ${release.publishedAt.slice(0, 10)}` : ""
      console.log(`latest release: ${release.tag}${dateStr}`)
    } else {
      console.log("latest release: unknown (network error?)")
    }
    return
  }

  const source = fromPath ? path.resolve(fromPath) : (release?.tag ? `github:carlkibler/toolsmith#${release.tag}` : "github:carlkibler/toolsmith")
  const sourceLabel = fromPath ? `local path ${source}` : `GitHub release ${release?.tag || "(latest)"}`
  ...
  console.log("  Removing previous Toolsmith install...")
  quietSpawn("npm", ["uninstall", "-g", "@carlkibler/toolsmith", "--silent"])
  if (process.exitCode) return

  console.log(`  Installing ${sourceLabel}...`)
  if (!quietSpawn("npm", ["install", "-g", source, "--silent"])) return

  if (latestVersion && !fromPath) console.log(`\nInstalled release: v${latestVersion}`)
  else console.log("\nInstalled.")
  if (!refreshInstalledSetup()) return
  console.log("\nDone. Run 'toolsmith doctor --smoke --online' to verify.")
}
