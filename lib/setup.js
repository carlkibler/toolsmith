...
export async function runSetup() {
  ...
  // Tripwire is ON by default (the active nudge most users never opt into). --no-tripwire skips;
  // --tripwire-mode picks firmness (adaptive escalation by default).
  const skipTripwire = args.includes("--no-tripwire")
  const tripwireMode = normalizeTripwireMode(option("--tripwire-mode") || "adaptive")
