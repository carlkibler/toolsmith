...
import { tryCommand, installContext, packageInfo, git, gitBranch, gitDirty, fetchAndGetAheadBehind, githubLatestRelease, realPathOrNull, MCP_BIN, backupConfigFile, parseClaudeToolsmithGet, configuredMcpPath, evaluateDrift, parseGeminiToolsmithConfig, parseCodexToolsmithConfig, mcpSmoke, stableNodeCommand } from "./config.js"
import { setupJsonMcpClientsAsync } from "./client-mcp.js"
import { setupPiResult } from "./pi.js"
import { installClaudeTripwire, installClaudePrime, removeClaudeTripwire, normalizeTripwireMode } from "./tripwire.js"
...
**Large file (>200 lines): Toolsmith first. Anchor before alteration.** Native Read with an explicit small range/limit up to ~300 lines is fine for inspection; use Toolsmith for broad reads, whole-file reads, and edits.
...
- Path: \`file_skeleton\` → \`find_and_anchor\` or \`get_function\` → \`anchored_edit\`. Avoid: whole-file Read → native Edit.
`
...
// Every instruction file a harness on this machine will actually read. Registering the
// MCP server is not enough on its own: omp inherits toolsmith's registration from the
// Claude/Codex configs and had listed its tools 42 times without ever calling one,
// because the priming that tells an agent to prefer them never reached omp's own
// AGENTS.md. Tools without priming are tools nobody picks.
export function primingTargets({ home = homedir(), cwd = process.cwd() } = {}) {
  const targets = [path.join(home, ".claude", "CLAUDE.md")]
  if (existsSync(path.join(home, ".codex"))) targets.push(path.join(home, ".codex", "AGENTS.md"))
  if (existsSync(path.join(home, ".gemini"))) targets.push(path.join(home, ".gemini", "GEMINI.md"))
  // omp keeps its user-level instructions under ~/.omp/agent, alongside its own mcp.json.
  if (existsSync(path.join(home, ".omp", "agent"))) targets.push(path.join(home, ".omp", "agent", "AGENTS.md"))
  const homeAgents = path.join(home, "AGENTS.md")
  if (existsSync(homeAgents)) targets.push(homeAgents)
  const localAgents = path.join(cwd, "AGENTS.md")
  if (existsSync(localAgents)) targets.push(localAgents)
  return [...new Set(targets)]
}

export function adoptInject() {
  for (const target of primingTargets()) {
...
export function setupClaudeResult(scope, force) {
  ...
  const result = tryClaude(["mcp", "add", "--scope", scope, "toolsmith", "--", stableNodeCommand(), MCP_BIN])
  if (result?.error) {
...
export function setupGeminiResult(force) {
  ...
  const result = tryGemini(["mcp", "add", "--scope", "user", "--trust", "toolsmith", stableNodeCommand(), MCP_BIN])
  if (result?.error) {
...
export function setupCodexResult(force) {
  ...
  const entry = `\n[mcp_servers.toolsmith]\n# ${provenanceTag()}\ncommand = ${JSON.stringify(stableNodeCommand())}\nargs = [${JSON.stringify(MCP_BIN)}]\n`
  try {
    backupConfigFile(codexConfig) // hand-edited TOML is the riskiest rewrite — keep a recoverable copy
...
export async function runSetup() {
  ...
  // Tripwire is ON by default but in nudge-only "allow" mode — it never prompts or blocks, just
  // suggests Toolsmith on large-file ops. Firmer modes (adaptive/ask/deny) are opt-in via
  // --tripwire-mode, since ask/deny inject permission prompts a user can't dismiss per-project.
  const skipTripwire = args.includes("--no-tripwire")
  const tripwireMode = normalizeTripwireMode(option("--tripwire-mode") || "allow")
  ...
  if (!skipPriming) {
    console.log("")
    adoptInject()
    const primePath = installClaudePrime()
    console.log(`  Toolsmith re-prime: SessionStart hook installed in ${primePath} (re-asserts the rule after compaction)`)
  }
  if (!skipTripwire) {
    const settingsPath = installClaudeTripwire(tripwireMode)
    console.log(`  Toolsmith tripwire: installed (mode: ${tripwireMode}) in ${settingsPath} — disable with 'toolsmith setup --no-tripwire'`)
  }
  if (skipSmoke) {
...
export function runAdopt() {
  ...
  if (inject) {
    ...
    for (const target of primingTargets()) removePrimingBlock(target)
    console.log("\nDone.")
  }
}
...
export function runUpdate() {
  ...
  process.stdout.write(`Updating toolsmith from ${sourceLabel}... `)
  quietSpawn("npm", ["uninstall", "-g", "@carlkibler/toolsmith", "--silent"])
  if (process.exitCode) return
  if (!quietSpawn("npm", ["install", "-g", source, "--silent"])) return

  const newVersion = globalBin ? (tryCommand(globalBin, ["--version"]) || "").trim() || null : null
  if (newVersion && newVersion !== currentVersion) {
    console.log(`v${currentVersion} → v${newVersion}`)
  } else if (newVersion) {
    console.log(`already on latest v${newVersion}`)
  } else {
