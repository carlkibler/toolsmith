...
export function setupPiResult(force = false) {
  ...
  if (health.state === "drift") {
    ...
    for (const entry of health.staleEntries || []) {
      ...
      if (remove.status !== 0) {
        ...
        failedRemovals.push({ entry, msg })
      }
    }
    if (failedRemovals.length > 0) {
      const pruned = prunePiPackageEntries(config, failedRemovals.map((item) => item.entry))
      notes.push(pruned ? `pruned ${failedRemovals.length} stale package ${failedRemovals.length === 1 ? "entry" : "entries"}` : "stale package removal failed")
      if (!pruned && verboseOutput()) notes.push(failedRemovals.map((item) => `${item.entry.source}: ${item.msg}`).join("; "))
    } else {
      notes.push(`removed ${(health.staleEntries || []).length} stale package ${(health.staleEntries || []).length === 1 ? "entry" : "entries"}`)
    }
    if (verboseOutput()) notes.unshift(`drift: ${installed} → ${health.expectedRealPath}`)
  }
  ...
  if (result.status === 0) {
    ...
    messages.push(`Pi.dev: ${health.state === "missing" ? "installed" : "refreshed"}${label ? ` ${label}` : ""}${suffix}`)
