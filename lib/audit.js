...
// counterfactual (the agent would have read the whole file). Edit-family savings settle
// against the same per-file ledger, so an edit that follows a read in the same session
// earns nothing extra — the read already booked the avoided load.
export function savingsBreakdownLine(summary) {
  ...
  return `  └─ read/search/skeleton: ${read} tokens · edit-family: ${edit} tokens across ${summary.agentEditCalls} edit(s)`
}

// Failed calls are the speed story the token counters miss: each one buys nothing and
// costs a full model round trip to retry.
export function failureLines(summary) {
  const failed = summary.agentFailedCalls || 0
  if (!failed) return []
  const rate = summary.agentToolCalls ? (failed / summary.agentToolCalls * 100).toFixed(1) : "0.0"
  const lines = [`failed agent calls: ${failed} (${rate}% — each costs a retry turn)`]
  const codes = Object.entries(summary.errorCodes || {}).sort((a, b) => b[1] - a[1])
  if (codes.length) lines.push(`  └─ reasons: ${codes.map(([code, count]) => `${code}=${count}`).join(", ")}`)
  const noCredit = Object.entries(summary.noCredit || {}).sort((a, b) => b[1] - a[1])
  if (noCredit.length) lines.push(`  └─ calls credited zero savings: ${noCredit.map(([reason, count]) => `${reason}=${count}`).join(", ")}`)
  return lines
}
...
export function clientAdoptionHints(summary) {
  const clients = {
    ...
    gemini: ["gemini", "gemini-cli-mcp-client"],
  }
  const countAny = (counts = {}, names = []) => names.reduce((sum, name) => sum + (counts[name] || 0), 0)

  return Object.entries(clients)
    .map(([client, aliases]) => ({
      client,
      aliases,
      starts: countAny(summary.agentStartupClients, aliases) || (summary.agentStartupClients[client] || 0),
      calls: countAny(summary.agentClients, aliases),
    }))
    .filter(({ starts, calls }) => starts > 0 && calls === 0)
    .map(({ client, aliases, starts }) => `${client}: MCP server started ${starts} non-test time(s), but no non-test tool calls recorded by ${aliases.join("/")}`)
}
...
export async function runAudit() {
  ...
  for (const line of failureLines(summary)) console.log(line)
  ...
  if (tripwire.total && Object.keys(tripwire.byDecision || {}).length) {
    console.log(`tripwire decisions: ${formatCounts(tripwire.byDecision, 6)}; modes: ${formatCounts(tripwire.byMode, 6)}; estimated retry-turn prompts: ${tripwire.firmPrompts || 0}`)
  }
...
export async function runTrends() {
  ...
  // ── round-trip cost ─────────────────────────────────────────────────────────
  // Tokens are only half the ledger. Every toolsmith call is a model round trip, and
  // a chain of skeleton → find → read → edit costs turns a native Read+Edit would not.
  // Savings bought with turns the user waits through is not obviously a good trade,
  // so report the turns next to the tokens instead of leaving them invisible.
  ...
  for (const record of agentRecords) {
    ...
    if (path) {
      const key = `${record.sessionId}|${record.cwdHash}|${path}`
      const entry = perFile.get(key) || { calls: 0, edits: 0 }
      entry.calls += 1
      if (/edit|replace/.test(record.tool || "")) entry.edits += 1
      perFile.set(key, entry)
    }
    const at = Date.parse(record.ts || "")
    if (Number.isFinite(at)) {
      const previous = lastCallBySession.get(record.sessionId)
      if (previous !== undefined) {
        const gap = (at - previous) / 1000
        if (gap >= 0 && gap <= 300) gaps.push(gap)
      }
      lastCallBySession.set(record.sessionId, at)
    }
  }
  const files = [...perFile.values()]
  const editedFiles = files.filter((entry) => entry.edits > 0)
  const failedCalls = agentRecords.filter((record) => record.error || record.result?.isError).length
  // Native cost is one Read to orient plus one Edit per change — NOT a flat 2. A file
  // that legitimately takes six edits would take six native Edit calls too, and counting
  // those as toolsmith overhead would manufacture a problem that isn't there. Only
  // navigation beyond the first orienting read is overhead.
  const nativeBaseline = (entry) => (entry.edits ? 1 + entry.edits : 1)
  const extraCalls = files.reduce((sum, entry) => sum + Math.max(0, entry.calls - nativeBaseline(entry)), 0)
  ...
  if (files.length) {
    ...
    console.log(`${"─".repeat(72)}\n`)
    const perEdited = editedFiles.length ? editedFiles.reduce((sum, entry) => sum + entry.calls, 0) / editedFiles.length : 0
    console.log(`  files touched:               ${nn(files.length)} (${nn(editedFiles.length)} edited)`)
    const perEditedNative = editedFiles.length ? editedFiles.reduce((sum, entry) => sum + nativeBaseline(entry), 0) / editedFiles.length : 0
    console.log(`  calls per edited file:       ${perEdited.toFixed(1)}  (native equivalent ~${perEditedNative.toFixed(1)}: one read + one edit per change)`)
    console.log(`  calls beyond native:         ${nn(extraCalls)}`)
    console.log(`  median gap between calls:    ${medianGap.toFixed(1)}s  (one model round trip)`)
    if (extraCalls && medianGap) console.log(`  added wall-clock (upper bd): ${(extraCalls * medianGap / 3600).toFixed(1)}h`)
    if (failedCalls) {
      const wasted = (failedCalls * medianGap / 3600).toFixed(1)
      console.log(`  failed calls:                ${nn(failedCalls)} (${pp(agentRecords.length ? failedCalls / agentRecords.length * 100 : 0)}) — ~${wasted}h of pure retry`)
    }
  }
