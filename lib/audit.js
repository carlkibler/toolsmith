...
export async function runTrends() {
  ...
  const missedCtxTokens = Math.round(lostLines * 12.5)
  const missedSavings = Math.round(missedCtxTokens * 0.7)
  ...
  console.log(`  tokens caught by toolsmith:  ${nn(totalCaught)} (${pp(totalFull > 0 ? totalCaught / totalFull * 100 : 0)} reduction on intercepted calls)`)
  console.log(`  estimated missed savings:    ${nn(missedSavings)}  (${sessionScan?.lostOpportunities?.total || 0} lost opportunities in session logs)`)
  if (totalOpportunity > 0) {
    console.log(`  interception rate:           ${pp(interceptionRate)}  (toolsmith caught this share of all large-file savings potential)`)
    console.log(`\n  honest overall picture: on every large-file operation, toolsmith saves ~${pp(totalFull > 0 ? totalCaught / totalFull * 100 : 0)}`)
    console.log(`  of the tokens when used. But it's only used on ~${pp(interceptionRate)} of the total opportunity.`)
