...
export async function runTrends() {
  ...
  // Native cost is one Read for a file only inspected, Read+Edit for one changed.
  const extraCalls = files.reduce((sum, entry) => sum + Math.max(0, entry.calls - (entry.edits ? 2 : 1)), 0)
  ...
  if (files.length) {
    ...
    console.log(`  calls per edited file:       ${perEdited.toFixed(1)}  (native Read+Edit is ~2)`)
