...
import { MCP_BIN, backupConfigFile, configuredMcpPath, evaluateDrift, realPathOrNull, tryCommand, stableNodeCommand } from "./config.js"
...
function writeJsonAtomic(configPath, data) {
  ...
  backupConfigFile(configPath) // recoverable copy before we replace the user's config
...
export function toolsmithMcpServerConfig(kind, current = {}) {
  if (kind === "opencode") {
    return {
      ...current,
      type: "local",
      command: [stableNodeCommand(), MCP_BIN],
      enabled: true,
    }
  }
  if (kind === "kilo-cli") {
...
export function doctorJsonMcpClients(expectedMcpPath) {
  return clientMcpTargets().map((target) => {
    ...
    return { state: "ok", label: target.label, path: target.path, message: drift.message }
  })
}
