...
export function symbolReplace({ path, content, store, sessionId, workspaceKey, name, search, replacement = "", regex = false, replaceAll = false, caseSensitive = true, commitAnchors = true }) {
  ...
  if (!range) {
    return { ok: false, notFound: true, notFoundKind: "symbol", path, content, errors: [`symbol not found: ${name}`], matches: 0, changed: false, beforeHash: contentHash(content), afterHash: contentHash(content) }
  }
  ...
  if (matches === 0) {
    return { ok: false, notFound: true, notFoundKind: "search", path, content, errors: [`search not found in symbol ${name}: ${search}`], symbolStartLine: range.startIndex + 1, symbolEndLine: range.endIndex + 1, matches: 0, changed: false, beforeHash: contentHash(content), afterHash: contentHash(content) }
  }
