...
- Harness-independent core in `src/`
  ...
  - non-enforcing adoption guidance at tool-selection and result time: large-file tool descriptions state the default path, read/search results include one short `Next:` action, and generated priming leads with `Toolsmith first. Anchor before alteration.`
...
- omp session logs are scanned from `~/.omp/agent/sessions` (JSONL; tool calls are `{type:"toolCall"}` blocks inside a message's `content`). Baseline on 2026-08-19 before priming: 1,418 reads of which 99.8% were unbounded whole-file reads, 550 edits, and zero toolsmith calls across 42 tool-list requests.
- omp (`oh-my-pi`) needs no MCP registration: it imports servers from `~/.claude/mcp.json` and `~/.codex/config.toml`, so toolsmith arrives automatically. It does need priming, which is why `primingTargets()` includes `~/.omp/agent/AGENTS.md`. Registration without priming is what produced 42 tool listings and zero calls.
